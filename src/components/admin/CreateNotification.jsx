...
export function CreateNotification({ token, onSuccess }) {
  ...
  return (
    <div className="rounded-lg border border-border/30 bg-card p-4"><h3 className="text-sm font-bold mb-3 flex items-center gap-1.5"><Bell className="w-5 h-5 text-purple-500" />Crear Notificación</h3>
      <form onSubmit={submit} className="space-y-3"><textarea placeholder="Mensaje..." className="w-full border border-hairline/60 bg-canvas px-3 py-2.5 rounded-sm text-sm focus:outline-none focus:border-primary/50 min-h-[70px]" value={message} onChange={e => setMessage(e.target.value)} required />
        <div className="grid grid-cols-3 gap-2"><select value={type} onChange={e => setType(e.target.value)} className="border border-hairline/60 bg-canvas px-3 py-2 rounded-sm text-sm"><option value="SYSTEM">📢 Sistema</option><option value="CONTEST">🏆 Contest</option><option value="RANK_UP">🚀 Rank Up</option><option value="WARNING">⚠️ Warning</option></select>
        <input type="text" placeholder="Link (opcional)" className="border border-hairline/60 bg-canvas px-3 py-2 rounded-sm text-sm focus:outline-none focus:border-primary/50" value={link} onChange={e => setLink(e.target.value)} />
        <input type="number" min="1" value={expireHours} onChange={e => setExpireHours(Number(e.target.value))} className="border border-hairline/60 bg-canvas px-3 py-2 rounded-sm text-sm focus:outline-none focus:border-primary/50" /></div>
