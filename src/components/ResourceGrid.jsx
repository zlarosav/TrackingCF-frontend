  ...
  return (
    <section className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        {items.map((r, i) => (
          <Card key={i} className="p-4 flex flex-col group transition-colors hover:bg-surface-elevated">
            <div className="flex items-start gap-3">
              {r.icon && <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-surface-elevated text-muted group-hover:text-primary transition-colors">{r.icon}</div>}
              <div className="flex-1 min-w-0">
                <h3 className="text-body-md font-semibold text-on-surface">{r.title}</h3>
