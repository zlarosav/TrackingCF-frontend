...
export default function UserTabs({ user, submissions, stats, handle }) {
  ...
  return (
    <div className="w-full space-y-5">
      <div className="flex rounded-xl bg-surface-card border border-hairline/60 p-0.5 overflow-x-auto">
        {tabs.map(({ id, label, icon: Icon }) => {
          const active = activeTab === id
          return (
            <button key={id} onClick={() => setActiveTab(id)}
              className={`relative flex items-center gap-1.5 px-4 py-2 text-body-md font-medium rounded-lg transition-all whitespace-nowrap ${active ? 'text-on-primary' : 'text-muted hover:text-on-surface'}`}>
