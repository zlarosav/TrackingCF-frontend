...
export default function StatsCard({ stats }) {
  const items = [
    ...
    { label: '1200+', value: stats.count_1200_plus || 0, icon: Award, color: 'text-indigo-500', score: '+5' },
  ]
  return (
      ...
      {items.map(({ label, value, icon: Icon, color, score }, i) => (
        <Card key={i} className="border-border/30">
          <CardContent className="p-3">
            <div className="flex items-center justify-between mb-1"><span className="text-xs font-medium text-muted-foreground">{label}</span><Icon className={`h-4 w-4 ${color}`} /></div>
